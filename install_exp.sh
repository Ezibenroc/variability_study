...
for host in $*; do {	
    ...
    run_command ${host} 'yes | apt install build-essential python python3 python3-dev zip linux-cpupower make'
    ...
    run_command ${host} 'cd scripts/cblas_tests && python3 ./multi_runner.py --nb_runs 10 --nb_calls 10 --size 100 -np 8 --csv_file /tmp/test.csv --lib openblas'
    echo "DONE for ${host}"
}&
done
wait
