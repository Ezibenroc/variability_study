...
for host in $*; do {	
    ...
    run_command ${host} 'yes | apt install build-essential python python3 python3-dev zip linux-cpupower make linux-tools git numactl likwid'
    ...
    run_command ${host} 'cd scripts/cblas_tests && python3 ./multi_runner.py --nb_runs 3 --nb_calls 10 --size 100 -np 1 --csv_file /tmp/test.csv --lib naive --remove_os_noise'
    run_command ${host} 'modprobe msr'
    run_command ${host} 'cd scripts/cblas_tests && python3 ./multi_runner.py --nb_runs 3 --nb_calls 10 --size 100 -np 1 --csv_file /tmp/test.csv --lib naive --likwid=clock'
    run_command ${host} 'cd scripts/cblas_tests && python3 ./multi_runner.py --nb_runs 3 --nb_calls 10 --size 100 -np 1 --csv_file /tmp/test.csv --lib naive --likwid=energy'
    echo "DONE for ${host}"
}&
done
wait
